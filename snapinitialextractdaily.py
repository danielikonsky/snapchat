...
# Define Snap Daily Extract csv file
                        
    cwd = os.getcwd()
    if not os.path.exists(cwd + '/snaptemp'):
        os.makedirs(cwd + '/snaptemp')
    csv_file = cwd + '/snaptemp/SnapExtractDaily.csv'    # <<<<<=== Change to a desirable output file location and/or name
...
# Get ad stats for each ad as a dataframe 
    ...
    adstats_df = adstats(token,granularity,ad_df,period,client_id,client_secret,grant_type,refresh_token)
...
def adstats(token,granul,ad_df,period,client_id,client_secret,grant_type,refresh_token):
    ...
    start_date = datetime.strptime('01/01/19', '%m/%d/%y')
    end_date = start_date + timedelta(30) 
    keep_running = True
    while end_date <  datetime.strptime('01/01/20', '%m/%d/%y'):
        ...
        if end_date >  datetime.strptime('12/31/19', '%m/%d/%y') and keep_running:
            end_date = datetime.strptime('12/31/19', '%m/%d/%y')
            keep_running = False
