...
# Get ad stats for each ad as a dataframe 
    ...
    adstats_df = adstats(token,granularity,ad_df,period)
...
def adstats(token,granul,ad_df,period):
    ...
    for i in range(0, len(ad_df)):
            ...
            tmend = str(timezone.utcoffset(datetime.now()))
            ...
            end_time = datetime.now().date().strftime('%Y-%m-%d')+"T" + str(endutcoffset) + ":00:00.000-00:00"
